...
package body Mathlib is
   ...
   procedure Extended_GCD(A, B : Integer_64; X, Y : out Integer_64; Result : out Integer_64) is
      X1, Y1, G : Integer_64;
   begin
      if B = 0 then
         X := 1;
   ...
   function Solve_Diophantine(A, B, C : Integer_64) return Diophantine_Solution is
      No_Sol : constant Diophantine_Solution := (0, 0, 0);
      X0, Y0, G, Scale, Step_X, Step_Y, T, X_Sol, Y_Sol : Integer_64;
   ...
   begin
      ...
      T_Min_X := Ceil_Div(1 - X0, Step_X);
      T_Min_Y := Ceil_Div(1 + Y0, Step_Y);
      ...
      if X_Sol > 0 and then Y_Sol > 0 then
         return (1, long_long(X_Sol), long_long(Y_Sol));
      else
         return No_Sol;
      end if;
